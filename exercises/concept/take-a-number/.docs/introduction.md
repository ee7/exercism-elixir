...
A message can be read from a mailbox using the `receive/1` macro. It accepts a `do` block that can [pattern match](https://exercism.org/tracks/elixir/concepts/pattern-matching) on the messages.
