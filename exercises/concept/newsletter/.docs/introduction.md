...
Every time a file is written to with `File.write/2`, a file descriptor is opened and a new Elixir [process](https://exercism.org/tracks/elixir/concepts/processes) is spawned. For this reason, writing to a file in a loop using `File.write/2` should be avoided.
